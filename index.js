...
APP.get("/:page?/:id?", (req, res) => {
    ...
    if (req.params.page === undefined) {
        ...
        Object.keys(DATA_BASE).forEach((key) => {
            if (key === "pages" || key === "main_page" || key === "admin")
                return;
        ...
        if (JSON.stringify(req.query) === "{}" && req.params.id === undefined) {
            ...
            const ITEM = RESULT.filter((item) => item.id === ID)[0];
            res.send(ITEM);
        }
        else {
            const ITEM = RESULT.filter((item) => item.page === req.query.page)[0];
            res.send(ITEM);
        }
    }
});
...
APP.put("/:page/:id", (req, res) => {
    const ID = /^\d+$/.test(req.params.id) ? Number(req.params.id) : req.params.id;
    const ITEM = DATA_BASE[req.params.page]
        .filter((item) => item.id === ID)[0];
    if (req.params.page === "pages") {
        const MAIN_PAGE_ITEM = DATA_BASE.main_page
